...
begin
  ...
  begin
    ...
    if Succeeded(apObjArray.Get('Hive', lFlags, pVal, pType, plFlavor)) and ((pType<>CIM_OBJECT) and (pType<>CIM_EMPTY) and (pType<>CIM_DATETIME) and (pType<>CIM_REFERENCE) )  then
    begin
      sValue:=pVal;
      VarClear(pVal);
      Writeln(Format('Hive %s',[sValue]));
    end;

    //You must convert to string the next types manually -> CIM_OBJECT, CIM_EMPTY, CIM_DATETIME, CIM_REFERENCE
    if Succeeded(apObjArray.Get('RootPath', lFlags, pVal, pType, plFlavor)) and ((pType<>CIM_OBJECT) and (pType<>CIM_EMPTY) and (pType<>CIM_DATETIME) and (pType<>CIM_REFERENCE) )  then
    begin
      sValue:=pVal;
      VarClear(pVal);
      Writeln(Format('RootPath %s',[sValue]));
    end;

    //You must convert to string the next types manually -> CIM_OBJECT, CIM_EMPTY, CIM_DATETIME, CIM_REFERENCE
    if Succeeded(apObjArray.Get('SECURITY_DESCRIPTOR', lFlags, pVal, pType, plFlavor)) and ((pType<>CIM_OBJECT) and (pType<>CIM_EMPTY) and (pType<>CIM_DATETIME) and (pType<>CIM_REFERENCE) )  then
    begin
      sValue:=pVal;
      VarClear(pVal);
      Writeln(Format('SECURITY_DESCRIPTOR %s',[sValue]));
    end;

    //You must convert to string the next types manually -> CIM_OBJECT, CIM_EMPTY, CIM_DATETIME, CIM_REFERENCE
    if Succeeded(apObjArray.Get('TIME_CREATED', lFlags, pVal, pType, plFlavor)) and ((pType<>CIM_OBJECT) and (pType<>CIM_EMPTY) and (pType<>CIM_DATETIME) and (pType<>CIM_REFERENCE) )  then
    begin
      sValue:=pVal;
      VarClear(pVal);
...
const
  ...
  strAuthority       = '';
  WQL ='Select * From RegistryTreeChangeEvent Within 1 ';
